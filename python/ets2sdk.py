...
class ets2sdkclient:
    def GetBool(self, i):
        if (i == Ets2SdkBoolean.TrailerAttached):
            return int(hex(self.flags[1]), 16) > 0
        else:
            return int(hex(self.aux[i]), 16) > 0
    ...
    def update(self):
        ...
        # [FieldOffset(8)]
        self.ets2_telemetry_plugin_revision = struct.unpack("I", self.mm[8:12])[
            0]
        # [FieldOffset(12)]
        ...
        self.trailerId = self.mm[172:236].decode("utf-8").replace("\x00", "")
        ...
        # [FieldOffset(308)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        self.jobCitySource = self.mm[308:372].decode(
            "utf-8").replace("\x00", "")

        # [FieldOffset(372)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        self.jobCityDestination = self.mm[372:436].decode(
            "utf-8").replace("\x00", "")

        # [FieldOffset(436)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        self.jobCompanySource = self.mm[436:500].decode(
            "utf-8").replace("\x00", "")

        # [FieldOffset(500)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        self.jobCompanyDestination = self.mm[500:564].decode(
            "utf-8").replace("\x00", "")
        ...
        # [FieldOffset(740)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        self.truckMakeId = self.mm[740:804].decode("utf-8").replace("\x00", "")
        # [FieldOffset(804)]
        #[MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        ...
        self.BlinkerRightActive = self.GetBool(
            Ets2SdkBoolean.BlinkerRightActive)
        ...
        self.BatteryVoltageWarning = self.GetBool(
            Ets2SdkBoolean.BatteryVoltageWarning)
        self.AirPressureWarning = self.GetBool(
            Ets2SdkBoolean.AirPressureWarning)
        self.AirPressureEmergency = self.GetBool(
            Ets2SdkBoolean.AirPressureEmergency)
        self.AdblueWarning = self.GetBool(Ets2SdkBoolean.AdblueWarning)
        self.OilPressureWarning = self.GetBool(
            Ets2SdkBoolean.OilPressureWarning)
        self.WaterTemperatureWarning = self.GetBool(
            Ets2SdkBoolean.WaterTemperatureWarning)
